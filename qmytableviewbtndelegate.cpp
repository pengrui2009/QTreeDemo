...
void QMyTableViewBtnDelegate::drawPushButton(QPainter* painter, const QStyleOptionViewItem& option, const QString&qsWndText) const
{
    ...
    // 设置按钮占据的矩形
    styleOptBtn.rect =  option.rect.adjusted(4, 4, -4, -4);
//    styleOptBtn.icon = qApp->style()->standardIcon(QStyle::SP_DesktopIcon); // 设置按钮图标
    // 设置按钮图标尺寸
//    styleOptBtn.iconSize = QSize(32, 32);
    // 设置按钮标题
    styleOptBtn.text = qsWndText;
    // 设置按钮状态
    styleOptBtn.state |= QStyle::State_Enabled;
    // 设置按钮水平布局，如果改为Qt::RightToLeft，则按钮图标在按钮标题右侧。
    styleOptBtn.direction = Qt::LeftToRight;
    // 设置按钮风格特点为普通扁平按钮
//    styleOptBtn.features = QStyleOptionButton::None | QStyleOptionButton::Flat;
...
void QMyTableViewBtnDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    // 计算按钮显示区域
    drawPushButton(painter, option, "选择数据");
#if 0
